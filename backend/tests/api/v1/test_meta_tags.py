...
@pytest.mark.asyncio
async def test_read_tags(authenticated_client: AsyncClient):
    response = await authenticated_client.get("/api/v1/tags/")
    assert response.status_code == 200
    assert isinstance(response.json(), list)

@pytest.mark.asyncio
async def test_create_and_delete_tag(authenticated_client: AsyncClient):
    # Create
    tag_data = {
    ...
    response = await authenticated_client.post("/api/v1/tags/", json=tag_data)
    ...
    if response.status_code == 200:
        ...
        # Get
        get_response = await authenticated_client.get("/api/v1/tags/")
        assert any(t["id"] == tag_id for t in get_response.json())

        # Delete
        del_response = await authenticated_client.delete(f"/api/v1/tags/{tag_id}")
